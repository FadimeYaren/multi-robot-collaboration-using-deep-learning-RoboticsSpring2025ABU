...
# 🎯 Auction-based görev atama fonksiyonu
def assign_tasks():
    for task in TASKS:
        # Eğer görev zaten atanmışsa geç
        ...
        for r in robots:
            if r['energy'] > 30 and r['target'] is None:
                ...
                bid = 2.0 * distance + 0.2 * (100 - r['energy'])  # ağırlıklar isteğe göre ayarlanabilir
                bids.append((bid, r))

        if bids:
            bids.sort(key=lambda x: x[0])  # en düşük bid kazanır
            chosen_robot = bids[0][1]
            chosen_robot['target'] = task

# Robotu hareket ettirir ve enerji durumunu günceller
...
def update(frame):
    ...
    ax.set_title("Battery-aware Task Assignment Simulation (Auction-Based)")
